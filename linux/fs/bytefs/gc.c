	...
	u64 curr_p, size_t *length)
{
	...
	struct bytefs_setattr_logentry *setattr_entry;
	struct bytefs_link_change_entry *linkc_entry;
	struct bytefs_mmap_entry *mmap_entry;
	struct bytefs_snapshot_info_entry *sn_entry;
	...
	if (metadata_csum == 0)
		...
		entryc = entry_copy;
		if (!bytefs_verify_entry_csum(sb, addr, entryc))
			return true;
	}
	...
	switch (type) {
	case SET_ATTR:
		setattr_entry = (struct bytefs_setattr_logentry *) entryc;
		if (setattr_entry->invalid == 0)
			ret = false;
		*length = sizeof(struct bytefs_setattr_logentry);
		break;
	case LINK_CHANGE:
		linkc_entry = (struct bytefs_link_change_entry *) entryc;
		if (linkc_entry->invalid == 0)
			ret = false;
		*length = sizeof(struct bytefs_link_change_entry);
		break;
	case FILE_WRITE:
		entry = (struct bytefs_file_write_entry *) entryc;
		if (entry->num_pages != entry->invalid_pages)
			ret = false;
		*length = sizeof(struct bytefs_file_write_entry);
		break;
	case DIR_LOG:
		dentry = (struct bytefs_dentry *) entryc;
		if (dentry->invalid == 0)
			ret = false;
		...
		*length = le16_to_cpu(dentry->de_len);
		break;
	case MMAP_WRITE:
		mmap_entry = (struct bytefs_mmap_entry *) entryc;
		if (mmap_entry->invalid == 0)
			ret = false;
		*length = sizeof(struct bytefs_mmap_entry);
		break;
	// case SNAPSHOT_INFO:
	// 	sn_entry = (struct bytefs_snapshot_info_entry *) entryc;
	// 	if (sn_entry->deleted == 0)
	// 		ret = false;
	// 	*length = sizeof(struct bytefs_snapshot_info_entry);
	// 	break;
	case NEXT_PAGE:
	...
	u64 page_head)
{
	...
	BYTEFS_START_TIMING(check_invalid_t, check_time);

	curr_page = (struct bytefs_inode_log_page *)
					bytefs_get_block(sb, page_head);
	rc = memcpy_mcsafe(&page_tail, &curr_page->page_tail,
					sizeof(struct bytefs_inode_page_tail));
	if (rc) {
		/* FIXME: Recover use replica log */
		bytefs_err(sb, "check page failed\n");
		return false;
	}
	...
	BYTEFS_END_TIMING(check_invalid_t, check_time);
	return ret;
}
...
static void free_curr_page(struct super_block *sb,
	...
	struct bytefs_inode_log_page *curr_page,
	struct bytefs_inode_log_page *last_page, u64 curr_head)
{
	u8 btype = sih->i_blk_type;
	unsigned long irq_flags = 0;

	bytefs_memunlock_block(sb, last_page, &irq_flags);
	bytefs_set_next_page_address(sb, last_page,
			curr_page->page_tail.next_page, 1);
	bytefs_memlock_block(sb, last_page, &irq_flags);
	bytefs_free_log_blocks(sb, sih,
			bytefs_get_blocknr(sb, curr_head, btype), 1);
}

static int bytefs_gc_assign_file_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih,
	struct bytefs_file_write_entry *old_entry,
	struct bytefs_file_write_entry *new_entry)
{
	struct bytefs_file_write_entry *temp;
	void **pentry;
...
static int bytefs_gc_assign_dentry(struct super_block *sb,
	struct bytefs_inode_info_header *sih, struct bytefs_dentry *old_dentry,
	struct bytefs_dentry *new_dentry)
{
	...
	bytefs_dbgv("%s: assign %s hash %lu\n", __func__,
	...
	if (found == 1 && hash == ret_node->hash) {
		if (ret_node->direntry == old_dentry)
			...
			bytefs_update_range_node_checksum(ret_node);    /* update checksum */
	}

	return ret;
}

static int bytefs_gc_assign_mmap_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih, u64 curr_p, u64 new_curr)
{
...
static int bytefs_gc_assign_snapshot_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih,
	struct bytefs_snapshot_info_entry *old_entry, u64 curr_p, u64 new_curr)
{
...
static int bytefs_gc_assign_new_entry(struct super_block *sb,
	struct bytefs_inode *pi, struct bytefs_inode_info_header *sih,
	u64 curr_p, u64 new_curr)
{
	struct bytefs_file_write_entry *old_entry, *new_entry;
	struct bytefs_dentry *old_dentry, *new_dentry;
	void *addr, *new_addr;
	u8 type;
	int ret = 0;
	...
	case LINK_CHANGE:
		...
		break;
	case MMAP_WRITE:
		ret = bytefs_gc_assign_mmap_entry(sb, sih, curr_p, new_curr);
		break;
	// case SNAPSHOT_INFO:
	// 	ret = bytefs_gc_assign_snapshot_entry(sb, sih, addr,
	// 					curr_p, new_curr);
	// 	break;
	case FILE_WRITE:
		new_addr = (void *)bytefs_get_block(sb, new_curr);
		old_entry = (struct bytefs_file_write_entry *)addr;
		new_entry = (struct bytefs_file_write_entry *)new_addr;
		ret = bytefs_gc_assign_file_entry(sb, sih, old_entry, new_entry);
		break;
	case DIR_LOG:
		new_addr = (void *)bytefs_get_block(sb, new_curr);
		old_dentry = (struct bytefs_dentry *)addr;
		new_dentry = (struct bytefs_dentry *)new_addr;
		if (sih->last_dentry == curr_p)
			sih->last_dentry = new_curr;
		ret = bytefs_gc_assign_dentry(sb, sih, old_dentry, new_dentry);
		break;
	default:
		bytefs_dbg("%s: unknown type %d, 0x%llx\n",
					__func__, type, curr_p);
...
static unsigned long bytefs_inode_log_thorough_gc(struct super_block *sb,
	struct bytefs_inode *pi, struct bytefs_inode_info_header *sih,
	unsigned long blocks, unsigned long checked_pages)
{
	struct bytefs_inode_log_page *curr_page = NULL;
	size_t length;
	...
	BYTEFS_START_TIMING(thorough_gc_t, gc_time);
	...
	allocated = bytefs_allocate_inode_log_pages(sb, sih, blocks,
					&new_head, ANY_CPU, 0);
	if (allocated != blocks) {
	...
	while (curr_p != sih->log_tail) {
		...
		if (!ret) {
			extended = 0;
			new_curr = bytefs_get_append_head(sb, pi, sih,
						new_curr, length, MAIN_LOG,
						1, &extended);
			if (extended)
				blocks++;
			/* Copy entry to the new log */
			bytefs_memunlock_block(sb, bytefs_get_block(sb, new_curr), &irq_flags);
			memcpy_to_pmem_nocache(bytefs_get_block(sb, new_curr),
				bytefs_get_block(sb, curr_p), length);
			bytefs_inc_page_num_entries(sb, new_curr);
			bytefs_memlock_block(sb, bytefs_get_block(sb, new_curr), &irq_flags);
			bytefs_gc_assign_new_entry(sb, pi, sih, curr_p, new_curr);
			new_curr += length;
		}
	...
	curr_page = (struct bytefs_inode_log_page *)bytefs_get_block(sb,
							BLOCK_OFF(new_curr));
	next = next_log_page(sb, new_curr);
	if (next > 0)
		bytefs_free_contiguous_log_blocks(sb, sih, next);

	bytefs_memunlock_block(sb, curr_page, &irq_flags);
	bytefs_set_next_page_flag(sb, new_curr);
	bytefs_set_next_page_address(sb, curr_page, tail_block, 0);
	...
	if (next != tail_block) {
		bytefs_err(sb, "Old log error: old curr_p 0x%lx, next 0x%lx ",
	...
	bytefs_free_contiguous_log_blocks(sb, sih, old_head);

	sih->log_pages = sih->log_pages + blocks - checked_pages;
	BYTEFS_STATS_ADD(thorough_gc_pages, checked_pages - blocks);
	BYTEFS_STATS_ADD(thorough_checked_pages, checked_pages);
out:
	BYTEFS_END_TIMING(thorough_gc_t, gc_time);
	return blocks;
}

/* Copy original log to alternate log */
static unsigned long bytefs_inode_alter_log_thorough_gc(struct super_block *sb,
	struct bytefs_inode *pi, struct bytefs_inode_info_header *sih,
	unsigned long blocks, unsigned long checked_pages)
{
	struct bytefs_inode_log_page *alter_curr_page = NULL;
	struct bytefs_inode *alter_pi;
	u64 ino = pi->bytefs_ino;
	u64 curr_p, new_curr;
	u64 alter_curr_p;
	...
	while (1) {
		bytefs_memunlock_block(sb, bytefs_get_block(sb, new_curr), &irq_flags);
		memcpy_to_pmem_nocache(bytefs_get_block(sb, new_curr),
				bytefs_get_block(sb, curr_p), LOG_BLOCK_TAIL);

		bytefs_set_alter_page_address(sb, curr_p, new_curr);
		bytefs_memlock_block(sb, bytefs_get_block(sb, new_curr), &irq_flags);
	...
	alter_curr_page = (struct bytefs_inode_log_page *)bytefs_get_block(sb,
							BLOCK_OFF(new_curr));
	alter_next = next_log_page(sb, new_curr);
	if (alter_next > 0)
		bytefs_free_contiguous_log_blocks(sb, sih, alter_next);
	bytefs_memunlock_block(sb, alter_curr_page, &irq_flags);
	bytefs_set_next_page_address(sb, alter_curr_page, alter_tail_block, 0);
	bytefs_memlock_block(sb, alter_curr_page, &irq_flags);
	...
	bytefs_memunlock_block(sb, alter_curr_page, &irq_flags);
	bytefs_set_next_page_address(sb, alter_curr_page, 0, 1);
	bytefs_memlock_block(sb, alter_curr_page, &irq_flags);

	/* Step 5: Free the old log */
	bytefs_free_contiguous_log_blocks(sb, sih, alter_old_head);
	...
	int num_pages, int force_thorough)
{
	...
	int found_head = 0;
	struct bytefs_inode_log_page *last_page = NULL;
	struct bytefs_inode_log_page *curr_page = NULL;
	struct bytefs_inode_log_page *alter_last_page = NULL;
	struct bytefs_inode_log_page *alter_curr_page = NULL;
	...
	BYTEFS_START_TIMING(fast_gc_t, gc_time);
	curr = sih->log_head;
	...
	bytefs_dbgv("%s: log head 0x%llx, tail 0x%llx\n",
	...
	while (1) {
		...
		curr_page = (struct bytefs_inode_log_page *)
					bytefs_get_block(sb, curr);
		next = next_log_page(sb, curr);
		if (next < 0)
			break;

		if (metadata_csum) {
			alter_curr_page = (struct bytefs_inode_log_page *)
						bytefs_get_block(sb, alter_curr);
			alter_next = next_log_page(sb, alter_curr);
			if (alter_next < 0)
				break;
		}
		bytefs_dbg_verbose("curr 0x%llx, next 0x%llx\n", curr, next);
		if (curr_page_invalid(sb, pi, sih, curr)) {
			bytefs_dbg_verbose("curr page %p invalid\n", curr_page);
			if (curr == sih->log_head) {
				...
				bytefs_dbg_verbose("Free log block 0x%llx\n",
						curr >> PAGE_SHIFT);
			...
			BYTEFS_STATS_ADD(fast_gc_pages, 1);
			freed_pages++;
		} else {
			if (found_head == 0) {
				possible_head = cpu_to_le64(curr);
	...
	BYTEFS_STATS_ADD(fast_checked_pages, checked_pages);
	bytefs_dbgv("checked pages %lu, freed %d\n", checked_pages, freed_pages);
	checked_pages -= freed_pages;

	// TODO:  I think this belongs in bytefs_extend_inode_log.
	if (num_pages > 0) {
		...
		if (metadata_csum) {
			...
			alter_curr_page = (struct bytefs_inode_log_page *)
				bytefs_get_block(sb, alter_curr);
			bytefs_memunlock_block(sb, curr_page, &irq_flags);
			bytefs_set_next_page_address(sb, alter_curr_page,
						   alter_new_block, 1);
			bytefs_memlock_block(sb, curr_page, &irq_flags);
		}
	}
	...
	bytefs_dbgv("%s: %d new head 0x%llx\n", __func__,
	...
	if (first_need_free) {
		bytefs_dbg_verbose("Free log head block 0x%llx\n",
					curr >> PAGE_SHIFT);
		bytefs_free_log_blocks(sb, sih,
				bytefs_get_blocknr(sb, curr, btype), 1);
		if (metadata_csum)
			bytefs_free_log_blocks(sb, sih,
				bytefs_get_blocknr(sb, alter_curr, btype), 1);
	}

	BYTEFS_END_TIMING(fast_gc_t, gc_time);
	...
	if (force_thorough || (blocks && blocks * 2 < checked_pages)) {
		bytefs_dbgv("Thorough GC for inode %lu: checked pages %lu, valid pages %lu\n",
				sih->ino,
				checked_pages, blocks);
		blocks = bytefs_inode_log_thorough_gc(sb, pi, sih,
							blocks, checked_pages);
		if (metadata_csum)
			bytefs_inode_alter_log_thorough_gc(sb, pi, sih,
							blocks, checked_pages);
	}
