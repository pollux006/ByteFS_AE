...
static int bytefs_calculate_block_parity(struct super_block *sb, u8 *parity,
	u8 *block)
{
...
static int bytefs_update_block_parity(struct super_block *sb,
	struct bytefs_inode_info_header *sih, void *block, unsigned long blocknr,
	size_t offset, size_t bytes, int zero)

 */
static int bytefs_update_block_parity(struct super_block *sb, u8 *block,
	unsigned long blocknr, int zero)
{
	...
	BYTEFS_START_TIMING(block_parity_t, block_parity_time);
	...
	if (block == NULL) {
		bytefs_dbg("%s: block pointer error\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	if (unlikely(zero))
		...
		bytefs_calculate_block_parity(sb, parity, block);

	nvmmptr = bytefs_get_parity_addr(sb, blocknr);

	bytefs_memunlock_range(sb, nvmmptr, strp_size, &irq_flags);
	memcpy_to_pmem_nocache(nvmmptr, parity, strp_size);
	bytefs_memlock_range(sb, nvmmptr, strp_size, &irq_flags);
...
out:
	...
	BYTEFS_END_TIMING(block_parity_t, block_parity_time);
	...
	unsigned long pgoff, int zero)
{
	...
	blocknr = bytefs_get_blocknr(sb, blockoff, sih->i_blk_type);
	bytefs_update_block_parity(sb, dax_mem, blocknr, zero);
	...
	size_t offset, size_t bytes)
{
	...
	size_t csum_size = BYTEFS_DATA_CSUM_LEN;
	size_t strp_size = BYTEFS_STRIPE_SIZE;
	unsigned int strp_shift = BYTEFS_STRIPE_SHIFT;
	...
	BYTEFS_STATS_ADD(block_csum_parity, 1);
	...
	if (unroll_csum || unroll_parity) {
		BYTEFS_START_TIMING(block_csum_parity_t, block_csum_parity_time);
		if (data_parity > 0) {
			parity = kmalloc(strp_size, GFP_KERNEL);
			if (parity == NULL) {
				bytefs_err(sb, "%s: buffer allocation error\n",
								__func__);
				ret = -ENOMEM;
				BYTEFS_END_TIMING(block_csum_parity_t,
		...
		if (data_csum > 0 && unroll_csum) {
			...
			nvmmptr = bytefs_get_data_csum_addr(sb, strp_nr, 0);
			nvmmptr1 = bytefs_get_data_csum_addr(sb, strp_nr, 1);
			bytefs_memunlock_range(sb, nvmmptr, csum_size * 8, &irq_flags);
			memcpy_to_pmem_nocache(nvmmptr, crc, csum_size * 8);
			memcpy_to_pmem_nocache(nvmmptr1, crc, csum_size * 8);
			bytefs_memlock_range(sb, nvmmptr, csum_size * 8, &irq_flags);
		}
		...
		if (parity != NULL)
			kfree(parity);
		BYTEFS_END_TIMING(block_csum_parity_t, block_csum_parity_time);
	}

	if (data_csum > 0 && !unroll_csum)
		bytefs_update_block_csum(sb, sih, block, blocknr,
					offset, bytes, 0);
	if (data_parity > 0 && !unroll_parity)
		bytefs_update_block_parity(sb, block, blocknr, 0);
	...
	u32 csum1, u32 *csum_good)
{
	...
	BYTEFS_START_TIMING(restore_data_t, restore_time);
	blockoff = bytefs_get_block_off(sb, blocknr, BYTEFS_BLOCK_TYPE_4K);
	...
	parity = bytefs_get_parity_addr(sb, blocknr);
	if (parity == NULL) {
		bytefs_err(sb, "%s: parity address error\n", __func__);
		ret = -EIO;
		goto out;
	}
	...
	bytefs_calculate_block_parity(sb, strip, block);
	for (i = 0; i < strp_size; i++) {
		...
		csum_calc = bytefs_crc32c(BYTEFS_INIT_CSUM, strip, strp_size);
	...
	if (success) {
		...
		bytefs_memunlock_range(sb, stripptr, strp_size, &irq_flags);
		memcpy_to_pmem_nocache(stripptr, strip, strp_size);
		bytefs_memlock_range(sb, stripptr, strp_size, &irq_flags);
...
out:
	...
	BYTEFS_END_TIMING(restore_data_t, restore_time);
	return ret;
}
	...
	struct inode *inode, loff_t newsize)
{
	...
	blocknr = bytefs_get_blocknr(sb, nvmm, btype);
