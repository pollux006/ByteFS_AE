...
#define __SUPER_H
/*
 * Structure of the BYTEFS super block in PMEM
 *
 ...
 * bytefs_get_block() returns NULL if the block offset is 0 (helps in catching
 ...
 * once the file system becomes stable and bytefs_get_block() returns correct
...
#define BYTEFS_SB_SIZE 512       /* must be power of two */
...
#define BYTEFS_ROOT_INO		(1)
#define BYTEFS_INODETABLE_INO	(2)	/* Fake inode associated with inode
...
#define BYTEFS_BLOCKNODE_INO	(3)     /* Storage for allocator state */
#define BYTEFS_LITEJOURNAL_INO	(4)     /* Storage for lightweight journals */
#define BYTEFS_INODELIST_INO	(5)     /* Storage for Inode free list */
#define BYTEFS_SNAPSHOT_INO	(6)	/* Storage for snapshot state */
#define BYTEFS_TEST_PERF_INO	(7)


/* Normal inode starts at 32 */
#define BYTEFS_NORMAL_INODE_START      (32)



/*
 * BYTEFS super-block data in DRAM
 */
struct bytefs_sb_info {
	struct super_block *sb;			/* VFS super block */
	struct bytefs_super_block *bytefs_sb;	/* DRAM copy of SB */
	struct block_device *s_bdev;
	struct dax_device *s_dax_dev;

	/*
	 * base physical and virtual address of BYTEFS (which is also
	 ...
	 * else do both
	 */
	unsigned int	bytefs_backing_option;
	...
	/* Snapshot related */
	struct bytefs_inode_info	*snapshot_si;
...
static inline struct bytefs_super_block
*bytefs_get_redund_super(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);

	return (struct bytefs_super_block *)(sbi->replica_sb_addr);
}


/* If this is part of a read-modify-write of the super block,
 * bytefs_memunlock_super() before calling!
 */
static inline struct bytefs_super_block *bytefs_get_super(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);

	return (struct bytefs_super_block *)sbi->virt_addr;
} //ANCHOR

extern struct super_block *bytefs_read_super(struct super_block *sb, void *data,
	int silent);
extern int bytefs_statfs(struct dentry *d, struct kstatfs *buf);
extern int bytefs_remount(struct super_block *sb, int *flags, char *data);
void *bytefs_ioremap(struct super_block *sb, phys_addr_t phys_addr,
	ssize_t size);
extern struct bytefs_range_node *bytefs_alloc_range_node_atomic(struct super_block *sb);
extern struct bytefs_range_node *bytefs_alloc_range_node(struct super_block *sb);
extern void bytefs_free_range_node(struct bytefs_range_node *node);
extern void bytefs_update_super_crc(struct super_block *sb);
extern void bytefs_sync_super(struct super_block *sb);
