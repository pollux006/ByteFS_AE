...
static int bytefs_data_csum_init_free_list(struct super_block *sb,
	struct free_list *free_list)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	 ...
	 */
	data_csum_blocks = ((sbi->initsize >> BYTEFS_STRIPE_SHIFT)
				* BYTEFS_DATA_CSUM_LEN) >> PAGE_SHIFT;
...
static int bytefs_data_parity_init_free_list(struct super_block *sb,
	struct free_list *free_list)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	...
	parity_blocks = total_blocks / (blocksize / BYTEFS_STRIPE_SIZE + 1);
	if (total_blocks % (blocksize / BYTEFS_STRIPE_SIZE + 1))
		parity_blocks++;
...
// manage.
static void bytefs_init_free_list(struct super_block *sb,
	struct free_list *free_list, int index)
{
	...
	bytefs_data_csum_init_free_list(sb, free_list);
	bytefs_data_parity_init_free_list(sb, free_list);
}
...
void bytefs_free_blocknode(struct bytefs_range_node *node)
{
	bytefs_free_range_node(node);
}


void bytefs_init_blockmap(struct super_block *sb, int recovery)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct rb_root *tree;                                                                            
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		bytefs_init_free_list(sb, free_list, i);
		...
		bytefs_dbgv("%s: free list %d: block start %lu, end %lu, "
			  "%lu free blocks\n",
			  __func__, i,
			  free_list->block_start,
			  free_list->block_end,
...
static inline int bytefs_rbtree_compare_rangenode(struct bytefs_range_node *curr,
...
int bytefs_find_range_node(struct rb_root *tree, unsigned long key,
	enum node_type type, struct bytefs_range_node **ret_node)
{
	struct bytefs_range_node *curr = NULL;
	struct rb_node *temp;
	int compVal;
	int ret = 0;
	...
	while (temp) {
		...
		compVal = bytefs_rbtree_compare_rangenode(curr, key, type);
	...
	if (curr && !bytefs_range_node_checksum_ok(curr)) {
		bytefs_dbg("%s: curr failed\n", __func__);
		return 0;
	}
...
int bytefs_insert_range_node(struct rb_root *tree,
	struct bytefs_range_node *new_node, enum node_type type)
{
	...
	while (*temp) {
		curr = container_of(*temp, struct bytefs_range_node, node);
		compVal = bytefs_rbtree_compare_rangenode(curr,
		...
		if (compVal == -1) {
			...
			bytefs_dbg("%s: type %d entry %lu - %lu already exists: "
				"%lu - %lu\n",
...
void bytefs_destroy_range_node_tree(struct super_block *sb,
	struct rb_root *tree)
{
	struct bytefs_range_node *curr;
	struct rb_node *temp;
	...
	struct bytefs_range_node *new_node)
{
	...
	ret = bytefs_insert_range_node(tree, new_node, NODE_BLOCK);
	if (ret)
...
{
	struct bytefs_range_node *ret_node = NULL;
	struct rb_node *tmp;
	int check_prev = 0, check_next = 0;
	int ret;

	ret = bytefs_find_range_node(tree, range_low, NODE_BLOCK, &ret_node);
	if (ret) {
		bytefs_dbg("%s ERROR: %lu - %lu already in free list\n",
	...
	if (!ret_node) {
		...
		*prev = ret_node;
		...
		if (tmp) {
			*next = container_of(tmp, struct bytefs_range_node, node);
			check_next = 1;
		} else {
			*next = NULL;
		}
		...
		*next = ret_node;
		...
		if (tmp) {
			*prev = container_of(tmp, struct bytefs_range_node, node);
			check_prev = 1;
		} else {
			*prev = NULL;
		}
	} else {
		bytefs_dbg("%s ERROR: %lu - %lu overlaps with existing "
			 "node %lu - %lu\n",
	...
	if (check_prev && !bytefs_range_node_checksum_ok(*prev)) {
		bytefs_dbg("%s: prev failed\n", __func__);
		return -EIO;
	}

	if (check_next && !bytefs_range_node_checksum_ok(*next)) {
		bytefs_dbg("%s: next failed\n", __func__);
		return -EIO;
	}
...
static int bytefs_free_blocks(struct super_block *sb, unsigned long blocknr,
	int num, unsigned short btype, int log_page)
{
	...
	struct bytefs_range_node *prev = NULL;
	struct bytefs_range_node *next = NULL;
	...
	if (num <= 0) {
		bytefs_dbg("%s ERROR: free %d\n", __func__, num);
		return -EINVAL;
	}

	BYTEFS_START_TIMING(free_blocks_t, free_time);
	cpuid = blocknr / sbi->per_list_blocks;

	/* Pre-allocate blocknode */
	curr_node = bytefs_alloc_blocknode(sb);
	if (curr_node == NULL) {
		/* returning without freeing the block*/
		BYTEFS_END_TIMING(free_blocks_t, free_time);
		return -ENOMEM;
	}
	...
	num_blocks = bytefs_get_numblocks(btype) * num;
	block_low = blocknr;
	block_high = blocknr + num_blocks - 1;

	bytefs_dbgv("Free: %lu - %lu\n", block_low, block_high);

	if (blocknr < free_list->block_start ||
			blocknr + num > free_list->block_end + 1) {
		bytefs_err(sb, "free blocks %lu to %lu, free list %d, "
			 "start %lu, end %lu\n",
	...
	ret = bytefs_find_free_slot(tree, block_low,
					block_high, &prev, &next);

	if (ret) {
		bytefs_dbg("%s: find free slot fail: %d\n", __func__, ret);
		goto out;
	}
	...
	new_node_used = 1;
	ret = bytefs_insert_blocktree(tree, curr_node);
	if (ret) {
		new_node_used = 0;
		goto out;
	}
...
out:
	...
	if (new_node_used == 0)
		bytefs_free_blocknode(curr_node);

	BYTEFS_END_TIMING(free_blocks_t, free_time);
	return ret;
}

int bytefs_free_data_blocks(struct super_block *sb,
	struct bytefs_inode_info_header *sih, unsigned long blocknr, int num)
{
	int ret;
	INIT_TIMING(free_time);

	bytefs_dbgv("Inode %lu: free %d data block from %lu to %lu\n",
			sih->ino, num, blocknr, blocknr + num - 1);
	if (blocknr == 0) {
		bytefs_dbg("%s: ERROR: %lu, %d\n", __func__, blocknr, num);
		return -EINVAL;
	}
	BYTEFS_START_TIMING(free_data_t, free_time);
	ret = bytefs_free_blocks(sb, blocknr, num, sih->i_blk_type, 0);
	if (ret) {
		bytefs_err(sb, "Inode %lu: free %d data block from %lu to %lu "
			 "failed!\n",
			 sih->ino, num, blocknr, blocknr + num - 1);
		bytefs_print_bytefs_log(sb, sih);
	}
	BYTEFS_END_TIMING(free_data_t, free_time);

	return ret;
}

int bytefs_free_log_blocks(struct super_block *sb,
	struct bytefs_inode_info_header *sih, unsigned long blocknr, int num)
{
	int ret;
	INIT_TIMING(free_time);

	bytefs_dbgv("Inode %lu: free %d log block from %lu to %lu\n",
			sih->ino, num, blocknr, blocknr + num - 1);
	if (blocknr == 0) {
		bytefs_dbg("%s: ERROR: %lu, %d\n", __func__, blocknr, num);
		return -EINVAL;
	}
	BYTEFS_START_TIMING(free_log_t, free_time);
	ret = bytefs_free_blocks(sb, blocknr, num, sih->i_blk_type, 1);
	if (ret) {
		bytefs_err(sb, "Inode %lu: free %d log block from %lu to %lu "
			 "failed!\n",
			 sih->ino, num, blocknr, blocknr + num - 1);
		bytefs_print_bytefs_log(sb, sih);
	}
	BYTEFS_END_TIMING(free_log_t, free_time);
...
{
	struct bytefs_range_node *first = free_list->first_node;
	struct bytefs_range_node *last = free_list->last_node;

	if (free_list->num_free_blocks < num_blocks || !first || !last) {
		bytefs_dbgv("%s: num_free_blocks=%ld; num_blocks=%ld; "
	...
	if (atype == LOG &&
		...
		bytefs_dbgv("%s: allocation would cause deadzone violation. "
...
struct bytefs_range_node *bytefs_alloc_blocknode_atomic(struct super_block *sb)
{
	return bytefs_alloc_range_node_atomic(sb);
}
...
static long bytefs_alloc_superpage(struct super_block *sb,
	struct free_list *free_list, unsigned long num_blocks,
	unsigned long *new_blocknr, enum bytefs_alloc_direction from_tail)
{
	struct rb_root *tree;
	struct rb_node *temp;
	struct bytefs_range_node *curr;
	...
	while (temp) {
		...
		if ((curr_blocks > left_margin) && \
			...
			*new_blocknr = curr->range_low + left_margin;
			...
			bytefs_dbgv("curr:%p: num_blocks:%lu curr->range_low:%lu high:%lu",
			...
			if (left_margin) {
				...
				bytefs_update_range_node_checksum(curr);
				bytefs_dbgv("Insert node for left_margin, range_low:%lu high:%lu",
			...
			if (right_margin) {
				if (left_margin) {
					...
					node = bytefs_alloc_blocknode_atomic(sb);
					if (node == NULL) {
						bytefs_warn("Failed to allocate new block node.\n");
						return -ENOMEM;
					}
					node->range_low = curr->range_low + left_margin + num_blocks;
					node->range_high = saved_range_high;
					bytefs_update_range_node_checksum(node);
					bytefs_insert_blocktree(tree, node);
					free_list->num_blocknode++;
				...
				bytefs_dbgv("Insert node for right_margin, range_low:%lu high:%lu",
			...
			if (!left_margin && !right_margin) {
				...
				if (curr == free_list->first_node) {
					temp = rb_next(temp);
					if (temp)
						node = container_of(temp, struct bytefs_range_node, node);
...
next:
	...
	BYTEFS_STATS_ADD(alloc_steps, step);
	return found ? num_blocks : 0;
}

/* Return how many blocks allocated */
static long bytefs_alloc_blocks_in_free_list(struct super_block *sb,
	struct free_list *free_list, unsigned short btype,
	enum alloc_type atype, unsigned long num_blocks,
	unsigned long *new_blocknr, enum bytefs_alloc_direction from_tail)
{
	struct rb_root *tree;
	struct bytefs_range_node *curr, *next = NULL, *prev = NULL;
	...
	if (!free_list->first_node || free_list->num_free_blocks == 0) {
		bytefs_dbgv("%s: Can't alloc. free_list->first_node=0x%p "
	...
	if (atype == LOG && not_enough_blocks(free_list, num_blocks, atype)) {
		bytefs_dbgv("%s: Can't alloc.  not_enough_blocks() == true",
			  __func__);
		return -ENOSPC;
	}
	...
	if (IS_DATABLOCKS_2MB_ALIGNED(num_blocks, atype)) {
		ret_blocks = bytefs_alloc_superpage(sb, free_list,
...
next:
	...
	if (free_list->num_free_blocks < num_blocks) {
		bytefs_dbg("%s: free list %d has %lu free blocks, "
...
success:
	if ((found == 1) || (found_hugeblock == 1))
		...
		bytefs_dbgv("%s: Can't alloc.  found = %d", __func__, found);
		return -ENOSPC;
	}
...
static int bytefs_get_candidate_free_list(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct free_list *free_list;
	int cpuid = 0;
...
static int bytefs_new_blocks(struct super_block *sb, unsigned long *blocknr,
	unsigned int num, unsigned short btype, int zero,
	enum alloc_type atype, int cpuid, enum bytefs_alloc_direction from_tail)
{
	struct free_list *free_list;
	void *bp;
	unsigned long num_blocks = 0;
	...
	num_blocks = num * bytefs_get_numblocks(btype);
	if (num_blocks == 0) {
		bytefs_dbg_verbose("%s: num_blocks == 0", __func__);
		return -EINVAL;
	}

	BYTEFS_START_TIMING(new_blocks_t, alloc_time);
	if (cpuid == ANY_CPU)
...
retry:
	...
	if (not_enough_blocks(free_list, num_blocks, atype)) {
		bytefs_dbgv("%s: cpu %d, free_blocks %lu, required %lu, "
			  "blocknode %lu\n",
		...
		spin_unlock(&free_list->s_lock);
		cpuid = bytefs_get_candidate_free_list(sb);
		retried++;
		goto retry;
	}
alloc:
	ret_blocks = bytefs_alloc_blocks_in_free_list(sb, free_list, btype, atype,
					num_blocks, &new_blocknr, from_tail);
	...
	spin_unlock(&free_list->s_lock);
	BYTEFS_END_TIMING(new_blocks_t, alloc_time);

	if (ret_blocks <= 0 || new_blocknr == 0) {
		bytefs_dbgv("%s: not able to allocate %d blocks. "
	...
	if (zero) {
		bp = bytefs_get_block(sb, bytefs_get_block_off(sb,
						new_blocknr, btype));
		bytefs_memunlock_range(sb, bp, PAGE_SIZE * ret_blocks, &irq_flags);
		memset_nt(bp, 0, PAGE_SIZE * ret_blocks);
		bytefs_memlock_range(sb, bp, PAGE_SIZE * ret_blocks, &irq_flags);
		// this should be sending a trim command not write 0 here
	}
	*blocknr = new_blocknr;

	bytefs_dbg_verbose("Alloc %lu NVMM blocks 0x%lx\n", ret_blocks, *blocknr);
	return ret_blocks / bytefs_get_numblocks(btype);
}

// Allocate data blocks.  The offset for the allocated block comes back in
// blocknr.  Return the number of blocks allocated.
int bytefs_new_data_blocks(struct super_block *sb,
	...
	enum bytefs_alloc_direction from_tail)
{
	...
	BYTEFS_START_TIMING(new_data_blocks_t, alloc_time);
	allocated = bytefs_new_blocks(sb, blocknr, num,
			    sih->i_blk_type, zero, DATA, cpu, from_tail);
	BYTEFS_END_TIMING(new_data_blocks_t, alloc_time);
	if (allocated < 0) {
		bytefs_dbgv("FAILED: Inode %lu, start blk %lu, "
			  "alloc %d data blocks from %lu to %lu\n",
			  sih->ino, start_blk, allocated, *blocknr,
			  *blocknr + allocated - 1);
	} else {
		bytefs_dbgv("Inode %lu, start blk %lu, "
			  "alloc %d data blocks from %lu to %lu\n",
			  sih->ino, start_blk, allocated, *blocknr,
			  *blocknr + allocated - 1);
	}
...
// blocknr.  Return the number of blocks allocated.
int bytefs_new_log_blocks(struct super_block *sb,
			...
			enum bytefs_alloc_direction from_tail)
{
	...
	BYTEFS_START_TIMING(new_log_blocks_t, alloc_time);
	allocated = bytefs_new_blocks(sb, blocknr, num,
			    sih->i_blk_type, zero, LOG, cpu, from_tail);
	BYTEFS_END_TIMING(new_log_blocks_t, alloc_time);
	if (allocated < 0) {
		bytefs_dbgv("%s: ino %lu, failed to alloc %d log blocks",
			  __func__, sih->ino, num);
	} else {
		bytefs_dbgv("%s: ino %lu, alloc %d of %d log blocks %lu to %lu\n",
...
unsigned long bytefs_count_free_blocks(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct free_list *free_list;
