...
static int to_flush_call(char *dst, char *src, size_t off, size_t size)
{
	/* pin src address to cache most reads, if size fits */
	/* dst address should point to pmem */
	bytefs_flush_buffer(dst + off, size, 0);
	return 0;
}
...
static u64 bytefs_crc32c_call(u64 init, char *data, size_t size)
{
	u32 crc = (u32) init;

	crc = bytefs_crc32c(crc, data, size);
	return (u64) crc;
}
...
static const checksum_call_t checksum_calls[] = {
	...
	{ "bytefs_crc32c",   bytefs_crc32c_call },
	{ "plain_xor64",   plain_xor64_call }
};

/* raid5 functions */
static u64 bytefs_block_parity_call(char **data, char *parity,
...
static u64 bytefs_block_csum_parity_call(char **data, char *parity,
	size_t size, int disks)
{
	int i;
	size_t strp_size = size;
	...
	for (i = 0; i < strp_size / 8; i++) {
		...
		// if (data_csum > 0 && unroll_csum) {
			bytefs_crc32c_qword(qwd[0], acc[0]);
			bytefs_crc32c_qword(qwd[1], acc[1]);
			bytefs_crc32c_qword(qwd[2], acc[2]);
			bytefs_crc32c_qword(qwd[3], acc[3]);
			bytefs_crc32c_qword(qwd[4], acc[4]);
			bytefs_crc32c_qword(qwd[5], acc[5]);
			bytefs_crc32c_qword(qwd[6], acc[6]);
			bytefs_crc32c_qword(qwd[7], acc[7]);
		// }
...
static const raid5_call_t raid5_calls[] = {
	...
	{ "bytefs_block_parity", bytefs_block_parity_call },
	{ "bytefs_block_csum_parity", bytefs_block_csum_parity_call },
//	{ "xor_blocks", xor_blocks_call },
};

/* memory pools for perf testing */
static void *bytefs_alloc_vmem_pool(size_t poolsize)
{
...
static void bytefs_free_vmem_pool(void *pool)
{
	if (pool != NULL)
		vfree(pool);
}

static void *bytefs_alloc_pmem_pool(struct super_block *sb,
	struct bytefs_inode_info_header *sih, int cpu, size_t poolsize,
	unsigned long *blocknr, int *allocated)
{
	int num;
	void *pool;
	size_t blocksize, blockoff;
	u8 blocktype = BYTEFS_BLOCK_TYPE_4K;
	...
	sih->ino = BYTEFS_TEST_PERF_INO;
	sih->i_blk_type = blocktype;
	sih->log_head = 0;
	sih->log_tail = 0;

	*allocated = bytefs_new_data_blocks(sb, sih, blocknr, 0, num,
					  ALLOC_NO_INIT, cpu, ALLOC_FROM_HEAD);
	if (*allocated < num) {
		bytefs_dbg("%s: allocated pmem blocks %d < requested blocks %d\n",
	...
	blockoff = bytefs_get_block_off(sb, *blocknr, blocktype);
	pool = bytefs_get_block(sb, blockoff);

	return pool;
}

static void bytefs_free_pmem_pool(struct super_block *sb,
	struct bytefs_inode_info_header *sih, char **pmem,
	unsigned long blocknr, int num)
{
	if (num > 0)
		bytefs_free_data_blocks(sb, sih, blocknr, num);
	*pmem = NULL;
}

static int bytefs_test_func_perf(struct super_block *sb, unsigned int func_id,
	size_t poolsize, size_t size, unsigned int disks)
{
	...
	struct bytefs_inode_info_header perf_sih;
	...
	if (call_id < NUM_MEMCPY_CALLS) {
		src = bytefs_alloc_vmem_pool(poolsize);
		dst = bytefs_alloc_vmem_pool(poolsize);
	...
	if (call_id < NUM_FROM_PMEM_CALLS) {
		pmem = bytefs_alloc_pmem_pool(sb, &perf_sih, cpu, poolsize,
							&blocknr, &allocated);
		dst = bytefs_alloc_vmem_pool(poolsize);
	...
	if (call_id < NUM_RAID5_CALLS) {
		...
		if (call_id == bytefs_block_csum_parity_id && disks != 8) {
			bytefs_dbg("%s only for 8 disks, skip testing\n", fname);
			goto out;
		}
...
test:
	if (fmemcpy == NULL && fchecksum == NULL && fraid5 == NULL) {
		bytefs_dbg("%s: function struct error\n", __func__);
		err = -EFAULT;
		goto out;
	}

	reset_perf_timer();
	BYTEFS_START_TIMING(perf_t, perf_time);
	...
	case from_pmem_gid:
		...
		break;
	case to_pmem_gid:
		bytefs_memunlock_range(sb, pmem, poolsize, &irq_flags);
		for (i = 0; i < reps; i++, off += size)
			err = fmemcpy->call(pmem, src, off, size);
		bytefs_memlock_range(sb, pmem, poolsize, &irq_flags);
		break;
	case checksum_gid:
		for (i = 0; i < reps; i++, off += size)
	...
	case raid5_gid:
		...
		break;
	default:
		bytefs_dbg("%s: invalid function group %d\n", __func__, call_gid);
		break;
	}

	BYTEFS_END_TIMING(perf_t, perf_time);
	nsec = read_perf_timer();

	// bytefs_info("checksum value: 0x%016llx\n", csum);
	...
	if (cpu != smp_processor_id()) /* scheduling shouldn't happen */
		bytefs_dbg("cpu was %d, now %d\n", cpu, smp_processor_id());

	bytefs_info("%4u %25s %4u %8lu %8lu\n", func_id, fname, cpu, lat, thru);

out:
	bytefs_free_vmem_pool(src);
	bytefs_free_vmem_pool(dst);
	bytefs_free_pmem_pool(sb, &perf_sih, &pmem, blocknr, allocated);
	...
	if (err)
		bytefs_dbg("%s: performance test aborted\n", __func__);
	return err;
}
	...
	unsigned int poolmb, size_t size, unsigned int disks)
{
	...
	if (!measure_timing) {
		bytefs_dbg("%s: measure_timing not set!\n", __func__);
		ret = -EFAULT;
		goto out;
	}
	if (func_id > NUM_PERF_CALLS) {
		bytefs_dbg("%s: invalid function id %d!\n", __func__, func_id);
		ret = -EFAULT;
		goto out;
	}
	if (poolmb < 1 || 1024 < poolmb) { /* limit pool size to 1GB */
		bytefs_dbg("%s: invalid pool size %u MB!\n", __func__, poolmb);
		ret = -EFAULT;
		goto out;
	}
	if (size < 64 || poolsize < size || (size % 64)) {
		bytefs_dbg("%s: invalid data size %zu!\n", __func__, size);
		ret = -EFAULT;
		goto out;
	}
	if (disks < 1 || 32 < disks) { /* limit number of disks */
		bytefs_dbg("%s: invalid disk count %u!\n", __func__, disks);
		ret = -EFAULT;
		goto out;
	}

	bytefs_info("test function performance\n");
	bytefs_info("pool size %u MB, work size %zu, disks %u\n",
					poolmb, size, disks);

	bytefs_info("%4s %25s %4s %8s %8s\n", "id", "name", "cpu", "ns", "MB/s");
	bytefs_info("-------------------------------------------------------\n");
	if (func_id == 0) {
		/* individual function id starting from 1 */
		for (id = 1; id <= NUM_PERF_CALLS; id++) {
			ret = bytefs_test_func_perf(sb, id, poolsize,
							size, disks);
			if (ret < 0)
				goto out;
		}
	} else {
		ret = bytefs_test_func_perf(sb, func_id, poolsize, size, disks);
	}
	bytefs_info("-------------------------------------------------------\n");
