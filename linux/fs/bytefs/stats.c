...
static void bytefs_print_alloc_stats(struct super_block *sb)
{
	...
	bytefs_info("=========== BYTEFS allocation stats ===========\n");
	bytefs_info("Alloc %llu, alloc steps %llu, average %llu\n",
		...
		Countstats[new_data_blocks_t] ?
			...
			: 0);
	bytefs_info("Free %llu\n", Countstats[free_data_t]);
	bytefs_info("Fast GC %llu, check pages %llu, free pages %llu, average %llu\n",
	...
	bytefs_info("Thorough GC %llu, checked pages %llu, free pages %llu, average %llu\n",
	...
	bytefs_info("alloc log count %lu, allocated log pages %lu, alloc data count %lu, allocated data pages %lu, free log count %lu, freed log pages %lu, free data count %lu, freed data pages %lu\n",
...
static void bytefs_print_IO_stats(struct super_block *sb)
{
	bytefs_info("=========== BYTEFS I/O stats ===========\n");
	bytefs_info("Read %llu, bytes %llu, average %llu\n",
		Countstats[dax_read_t], IOstats[read_bytes],
		Countstats[dax_read_t] ?
			IOstats[read_bytes] / Countstats[dax_read_t] : 0);
	bytefs_info("COW write %llu, bytes %llu, average %llu, write breaks %llu, average %llu\n",
		...
		IOstats[cow_write_breaks], Countstats[do_cow_write_t] ?
			IOstats[cow_write_breaks] / Countstats[do_cow_write_t]
			: 0);
	bytefs_info("Inplace write %llu, bytes %llu, average %llu, write breaks %llu, average %llu\n",
...
void bytefs_print_timing_stats(struct super_block *sb)
{
	...
	bytefs_get_timing_stats();
	bytefs_get_IO_stats();

	bytefs_info("=========== BYTEFS kernel timing stats ============\n");
	for (i = 0; i < TIMING_NUM; i++) {
		/* Title */
		if (Timingstring[i][0] == '=') {
			bytefs_info("\n%s\n\n", Timingstring[i]);
		...
		if (measure_timing || Timingstats[i]) {
			bytefs_info("%s: count %llu, timing %llu, average %llu\n",
				Timingstring[i],
				Countstats[i],
				Timingstats[i],
				Countstats[i] ?
	...
	bytefs_info("\n");
	bytefs_print_alloc_stats(sb);
	bytefs_print_IO_stats(sb);
}

static void bytefs_clear_timing_stats(void)
{
	int i;
	int cpu;
...
static void bytefs_clear_IO_stats(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct free_list *free_list;
	int i;
...
{
	bytefs_clear_timing_stats();
	bytefs_clear_IO_stats(sb);
}

void bytefs_print_inode(struct bytefs_inode *pi)
{
	bytefs_dbg("%s: BYTEFS inode %llu\n", __func__, pi->bytefs_ino);
	bytefs_dbg("valid %u, deleted %u, blk type %u, flags %u\n",
		pi->valid, pi->deleted, pi->i_blk_type, pi->i_flags);
	bytefs_dbg("size %llu, ctime %u, mtime %u, atime %u\n",
		pi->i_size, pi->i_ctime, pi->i_mtime, pi->i_atime);
	bytefs_dbg("mode %u, links %u, xattr 0x%llx, csum %u\n",
		pi->i_mode, pi->i_links_count, pi->i_xattr, pi->csum);
	bytefs_dbg("uid %u, gid %u, gen %u, create time %u\n",
		pi->i_uid, pi->i_gid, pi->i_generation, pi->i_create_time);
	bytefs_dbg("head 0x%llx, tail 0x%llx, alter head 0x%llx, tail 0x%llx\n",
		pi->log_head, pi->log_tail, pi->alter_log_head,
		pi->alter_log_tail);
	bytefs_dbg("create epoch id %llu, delete epoch id %llu\n",
		pi->create_epoch_id, pi->delete_epoch_id);
}

static inline void bytefs_print_file_write_entry(struct super_block *sb,
	u64 curr, struct bytefs_file_write_entry *entry)
{
	bytefs_dbg("file write entry @ 0x%llx: epoch %llu, trans %llu, pgoff %llu, pages %u, blocknr %llu, reassigned %u, updating %u, invalid count %u, size %llu, mtime %u\n",
...
static inline void bytefs_print_set_attr_entry(struct super_block *sb,
	u64 curr, struct bytefs_setattr_logentry *entry)
{
	bytefs_dbg("set attr entry @ 0x%llx: epoch %llu, trans %llu, invalid %u, mode %u, size %llu, atime %u, mtime %u, ctime %u\n",
			curr, entry->epoch_id, entry->trans_id,
			entry->invalid, entry->mode,
			entry->size, entry->atime, entry->mtime, entry->ctime);
}

static inline void bytefs_print_link_change_entry(struct super_block *sb,
	u64 curr, struct bytefs_link_change_entry *entry)
{
	bytefs_dbg("link change entry @ 0x%llx: epoch %llu, trans %llu, invalid %u, links %u, flags %u, ctime %u\n",
			curr, entry->epoch_id, entry->trans_id,
			entry->invalid, entry->links,
			entry->flags, entry->ctime);
}

static inline void bytefs_print_mmap_entry(struct super_block *sb,
	u64 curr, struct bytefs_mmap_entry *entry)
{
	bytefs_dbg("mmap write entry @ 0x%llx: epoch %llu, invalid %u, pgoff %llu, pages %llu\n",
			curr, entry->epoch_id, entry->invalid,
			entry->pgoff, entry->num_pages);
}

static inline void bytefs_print_snapshot_info_entry(struct super_block *sb,
	u64 curr, struct bytefs_snapshot_info_entry *entry)
{
	bytefs_dbg("snapshot info entry @ 0x%llx: epoch %llu, deleted %u, timestamp %llu\n",
			curr, entry->epoch_id, entry->deleted,
			entry->timestamp);
}

static inline size_t bytefs_print_dentry(struct super_block *sb,
	u64 curr, struct bytefs_dentry *entry)
{
	bytefs_dbg("dir logentry @ 0x%llx: epoch %llu, trans %llu, reassigned %u, invalid %u, inode %llu, links %u, namelen %u, rec len %u, name %s, mtime %u\n",
			curr, entry->epoch_id, entry->trans_id,
...
u64 bytefs_print_log_entry(struct super_block *sb, u64 curr)
{
	...
	addr = (void *)bytefs_get_block(sb, curr);
	type = bytefs_get_entry_type(addr);
	switch (type) {
	case SET_ATTR:
		bytefs_print_set_attr_entry(sb, curr, addr);
		curr += sizeof(struct bytefs_setattr_logentry);
		break;
	case LINK_CHANGE:
		bytefs_print_link_change_entry(sb, curr, addr);
		curr += sizeof(struct bytefs_link_change_entry);
		break;
	case MMAP_WRITE:
		bytefs_print_mmap_entry(sb, curr, addr);
		curr += sizeof(struct bytefs_mmap_entry);
		break;
	case SNAPSHOT_INFO:
		bytefs_print_snapshot_info_entry(sb, curr, addr);
		curr += sizeof(struct bytefs_snapshot_info_entry);
		break;
	case FILE_WRITE:
		bytefs_print_file_write_entry(sb, curr, addr);
		curr += sizeof(struct bytefs_file_write_entry);
		break;
	case DIR_LOG:
		size = bytefs_print_dentry(sb, curr, addr);
		curr += size;
		if (size == 0) {
			bytefs_dbg("%s: dentry with size 0 @ 0x%llx\n",
					__func__, curr);
			curr += sizeof(struct bytefs_file_write_entry);
			BYTEFS_ASSERT(0);
		}
		break;
	case NEXT_PAGE:
		bytefs_dbg("%s: next page sign @ 0x%llx\n", __func__, curr);
		curr = PAGE_TAIL(curr);
		break;
	default:
...
{
	struct bytefs_inode_page_tail *tail;
	u64 start, end;
	...
	while (start < end && start != 0)
		start = bytefs_print_log_entry(sb, start);

	tail = bytefs_get_block(sb, end);
	bytefs_dbg("Page tail. curr 0x%llx, next page 0x%llx, %u entries, %u invalid\n",
	...
	struct bytefs_inode_info_header *sih)
{
	...
	curr = sih->log_head;
	bytefs_dbg("Pi %lu: log head 0x%llx, tail 0x%llx\n",
			sih->ino, curr, sih->log_tail);

	if (sih->ino != BYTEFS_ROOT_INO && sih->ino < BYTEFS_NORMAL_INODE_START) {
		bytefs_dbg("Special inode, return\n");
		return;
	}
	...
	while (curr != sih->log_tail) {
		if ((curr & (PAGE_SIZE - 1)) == LOG_BLOCK_TAIL) {
			...
			bytefs_dbg("Log tail, curr 0x%llx, next page 0x%llx, %u entries, %u invalid\n",
					curr, tail->next_page,
					tail->num_entries,
					tail->invalid_entries);
			curr = tail->next_page;
		} else {
			curr = bytefs_print_log_entry(sb, curr);
		}
		if (curr == 0)
			break;
	}
	...
	struct bytefs_inode_info_header *sih)
{
	...
	curr_page = (struct bytefs_inode_log_page *)bytefs_get_block(sb, curr);
	while ((next = curr_page->page_tail.next_page) != 0) {
		bytefs_dbg("Current page 0x%llx, next page 0x%llx, %u entries, %u invalid\n",
	...
	if (sih->log_tail >> PAGE_SHIFT == curr >> PAGE_SHIFT)
		used = count;
	bytefs_dbg("Pi %lu: log used %d pages, has %d pages, si reports %lu pages\n",
		sih->ino, used, count,
		sih->log_pages);
}
...
int bytefs_check_inode_logs(struct super_block *sb, struct bytefs_inode *pi)
{
	...
	while (curr && alter_curr) {
		if (alter_log_page(sb, curr) != alter_curr ||
			...
			bytefs_dbg("Inode %llu page %d: curr 0x%llx, alter 0x%llx, alter_curr 0x%llx, alter 0x%llx\n",
					pi->bytefs_ino, count1,
	...
	bytefs_dbg("Log1 %d pages, tail @ page %d\n", count1, tail1_at);
	bytefs_dbg("Log2 %d pages, tail @ page %d\n", count2, tail2_at);
...
void bytefs_print_free_lists(struct super_block *sb)
{
	...
	bytefs_dbg("======== BYTEFS per-CPU free list allocation stats ========\n");
	for (i = 0; i < sbi->cpus; i++) {
		free_list = bytefs_get_free_list(sb, i);
		bytefs_dbg("Free list %d: block start %lu, block end %lu, num_blocks %lu, num_free_blocks %lu, blocknode %lu\n",
		...
		bytefs_dbg("Free list %d: csum start %lu, replica csum start %lu, csum blocks %lu, parity start %lu, parity blocks %lu\n",
		...
		bytefs_dbg("Free list %d: alloc log count %lu, allocated log pages %lu, alloc data count %lu, allocated data pages %lu, free log count %lu, freed log pages %lu, free data count %lu, freed data pages %lu\n",
			 i,
