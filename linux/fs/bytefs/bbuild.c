...
static inline int get_block_cpuid(struct bytefs_sb_info *sbi,
	unsigned long blocknr)
{
	return blocknr / sbi->per_list_blocks;
}

static int bytefs_failure_insert_inodetree(struct super_block *sb,
	unsigned long ino_low, unsigned long ino_high)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct inode_map *inode_map;
	struct bytefs_range_node *prev = NULL, *next = NULL;
	...
	if (ino_low > ino_high) {
		bytefs_err(sb, "%s: ino low %lu, ino high %lu\n",
	...
	ret = bytefs_find_free_slot(tree, internal_low, internal_high,
					&prev, &next);
	if (ret) {
		bytefs_dbg("%s: ino %lu - %lu already exists!: %d\n",
	...
	if (prev && next && (internal_low == prev->range_high + 1) &&
		...
		bytefs_update_range_node_checksum(prev);
		bytefs_free_inode_node(next);
		goto finish;
	}
	...
	if (next && (internal_high + 1 == next->range_low)) {
		...
		bytefs_update_range_node_checksum(next);
		goto finish;
	}

	/* Aligns somewhere in the middle */
	new_node = bytefs_alloc_inode_node(sb);
	BYTEFS_ASSERT(new_node);
	new_node->range_low = internal_low;
	new_node->range_high = internal_high;
	bytefs_update_range_node_checksum(new_node);
	ret = bytefs_insert_inodetree(sbi, new_node, cpu);
	if (ret) {
		bytefs_err(sb, "%s failed\n", __func__);
		bytefs_free_inode_node(new_node);
		goto finish;
	}
	inode_map->num_range_node_inode++;
...
static void bytefs_destroy_blocknode_tree(struct super_block *sb, int cpu)
{
	struct free_list *free_list;

	free_list = bytefs_get_free_list(sb, cpu);
	bytefs_destroy_range_node_tree(sb, &free_list->block_free_tree);
}

static void bytefs_destroy_blocknode_trees(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	int i;

	for (i = 0; i < sbi->cpus; i++)
		bytefs_destroy_blocknode_tree(sb, i);

}

static int bytefs_init_blockmap_from_inode(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct bytefs_inode *pi = bytefs_get_inode_by_ino(sb, BYTEFS_BLOCKNODE_INO);
	struct bytefs_inode_info_header sih;
	struct free_list *free_list;
	struct bytefs_range_node_lowhigh *entry;
	struct bytefs_range_node *blknode;
	size_t size = sizeof(struct bytefs_range_node_lowhigh);
	u64 curr_p;
	u64 cpuid;
	int ret = 0;

	memset(&sih, 0, sizeof(struct bytefs_inode_info_header));
	...
	sih.ino = BYTEFS_BLOCKNODE_INO;
	curr_p = sih.log_head;
	if (curr_p == 0) {
		bytefs_dbg("%s: pi head is 0!\n", __func__);
		return -EINVAL;
	}
	...
	while (curr_p != sih.log_tail) {
		...
		if (curr_p == 0) {
			bytefs_dbg("%s: curr_p is NULL!\n", __func__);
			BYTEFS_ASSERT(0);
			ret = -EINVAL;
			break;
		}

		entry = (struct bytefs_range_node_lowhigh *)bytefs_get_block(sb,
							curr_p);
		blknode = bytefs_alloc_blocknode(sb);
		if (blknode == NULL)
			BYTEFS_ASSERT(0);
		blknode->range_low = le64_to_cpu(entry->range_low);
		blknode->range_high = le64_to_cpu(entry->range_high);
		bytefs_update_range_node_checksum(blknode);
		cpuid = get_block_cpuid(sbi, blknode->range_low);

		/* FIXME: Assume NR_CPUS not change */
		free_list = bytefs_get_free_list(sb, cpuid);
		ret = bytefs_insert_blocktree(&free_list->block_free_tree,
						blknode);
		if (ret) {
			bytefs_err(sb, "%s failed\n", __func__);
			bytefs_free_blocknode(blknode);
			BYTEFS_ASSERT(0);
			bytefs_destroy_blocknode_trees(sb);
			goto out;
		}
		...
		curr_p += sizeof(struct bytefs_range_node_lowhigh);
	}
out:
...
static void bytefs_destroy_inode_trees(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct inode_map *inode_map;
	int i;
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		bytefs_destroy_range_node_tree(sb,
...
static int bytefs_init_inode_list_from_inode(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct bytefs_inode *pi = bytefs_get_inode_by_ino(sb, BYTEFS_INODELIST_INO);
	struct bytefs_inode_info_header sih;
	struct bytefs_range_node_lowhigh *entry;
	...
	while (curr_p != sih.log_tail) {
		...
		if (cpuid >= sbi->cpus) {
			bytefs_err(sb, "Invalid cpuid %lu\n", cpuid);
			bytefs_free_inode_node(range_node);
			BYTEFS_ASSERT(0);
			bytefs_destroy_inode_trees(sb);
			goto out;
		}
		...
		ret = bytefs_insert_inodetree(sbi, range_node, cpuid);
		if (ret) {
			bytefs_err(sb, "%s failed, %d\n", __func__, cpuid);
			bytefs_free_inode_node(range_node);
			BYTEFS_ASSERT(0);
			bytefs_destroy_inode_trees(sb);
			goto out;
	...
	bytefs_dbg("%s: %lu inode nodes\n", __func__, num_inode_node);
out:
	bytefs_free_inode_log(sb, pi, &sih);
	return ret;
}

static u64 bytefs_append_range_node_entry(struct super_block *sb,
	struct bytefs_range_node *curr, u64 tail, unsigned long cpuid)
{
	u64 curr_p;
	size_t size = sizeof(struct bytefs_range_node_lowhigh);
	struct bytefs_range_node_lowhigh *entry;
	...
	if (!bytefs_range_node_checksum_ok(curr)) {
		bytefs_dbg("%s: range node checksum failure\n", __func__);
		goto out;
	}

	if (curr_p == 0 || (is_last_entry(curr_p, size) &&
				next_log_page(sb, curr_p) == 0)) {
		bytefs_dbg("%s: inode log reaches end?\n", __func__);
		goto out;
	}
	...
	entry = (struct bytefs_range_node_lowhigh *)bytefs_get_block(sb, curr_p);
	...
	bytefs_dbgv("append entry block low 0x%lx, high 0x%lx\n",
			curr->range_low, curr->range_high);

	bytefs_write(entry,64);
	bytefs_flush_buffer(entry, sizeof(struct bytefs_range_node_lowhigh), 0);
out:
	return curr_p;
}

static u64 bytefs_save_range_nodes_to_log(struct super_block *sb,
	struct rb_root *tree, u64 temp_tail, unsigned long cpuid)
{
	...
	while (temp) {
		...
		curr_entry = bytefs_append_range_node_entry(sb, curr,
						temp_tail, cpuid);
		temp_tail = curr_entry + size;
		temp = rb_next(temp);
		rb_erase(&curr->node, tree);
		bytefs_free_range_node(curr);
	}

	return temp_tail;
}

static u64 bytefs_save_free_list_blocknodes(struct super_block *sb, int cpu,
	u64 temp_tail)
{
	struct free_list *free_list;

	free_list = bytefs_get_free_list(sb, cpu);
	temp_tail = bytefs_save_range_nodes_to_log(sb,
...
void bytefs_save_inode_list_to_log(struct super_block *sb)
{
	...
	allocated = bytefs_allocate_inode_log_pages(sb, &sih, num_blocks,
						&new_block, ANY_CPU, 0);
	if (allocated != num_blocks) {
		bytefs_dbg("Error saving inode list: %d\n", allocated);
		return;
	}
	...
	bytefs_update_tail(pi, temp_tail);
	// we only do this when append
	bytefs_write(&pi->log_head,64);
	bytefs_flush_buffer(&pi->log_head, CACHELINE_SIZE, 0);
	bytefs_memlock_inode(sb, pi, &irq_flags);

	bytefs_dbg("%s: %lu inode nodes, pi head 0x%llx, tail 0x%llx\n",
...
void bytefs_save_blocknode_mappings_to_log(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		free_list = bytefs_get_free_list(sb, i);
		num_blocknode += free_list->num_blocknode;
		bytefs_dbgv("%s: free list %d: %lu nodes\n", __func__,
	...
	allocated = bytefs_allocate_inode_log_pages(sb, &sih, num_pages,
						&new_block, ANY_CPU, 0);
	if (allocated != num_pages) {
		bytefs_dbg("Error saving blocknode mappings: %d\n", allocated);
		return;
	}

	temp_tail = new_block;
	for (i = 0; i < sbi->cpus; i++)
		temp_tail = bytefs_save_free_list_blocknodes(sb, i, temp_tail);
	...
	bytefs_dbg("%s: %lu blocknodes, %lu log pages, pi head 0x%llx, tail 0x%llx\n",
		  __func__, num_blocknode, num_pages,
		  pi->log_head, pi->log_tail);
}

static int bytefs_insert_blocknode_map(struct super_block *sb,
	int cpuid, unsigned long low, unsigned long high)
{
	struct free_list *free_list;
	struct rb_root *tree;
	struct bytefs_range_node *blknode = NULL;
	unsigned long num_blocks = 0;
	int ret;

	num_blocks = high - low + 1;
	bytefs_dbgv("%s: cpu %d, low %lu, high %lu, num %lu\n",
	...
	bytefs_update_range_node_checksum(blknode);
	ret = bytefs_insert_blocktree(tree, blknode);
	if (ret) {
		bytefs_err(sb, "%s failed\n", __func__);
		bytefs_free_blocknode(blknode);
		goto out;
...
static int __bytefs_build_blocknode_map(struct super_block *sb,
	unsigned long *bitmap, unsigned long bsize, unsigned long scale)
{
	...
	while (1) {
		...
		if (bytefs_insert_blocknode_map(sb, cpuid,
				low << scale, (next << scale) - 1)) {
			bytefs_dbg("Error: could not insert %lu - %lu\n",
				low << scale, ((next << scale) - 1));
		}
		start = next;
		if (next == bsize)
...
static void bytefs_update_4K_map(struct super_block *sb,
...
static int bytefs_build_blocknode_map(struct super_block *sb,
	unsigned long initsize)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		bm = global_bm[i];
		bytefs_update_4K_map(sb, bm, bm->scan_bm_2M.bitmap,
			bm->scan_bm_2M.bitmap_size * 8, PAGE_SHIFT_2M - 12);
		bytefs_update_4K_map(sb, bm, bm->scan_bm_1G.bitmap,
	...
	ret = __bytefs_build_blocknode_map(sb, final_bm->scan_bm_4K.bitmap,
...
static int bytefs_traverse_inode_log(struct super_block *sb,
	struct bytefs_inode *pi, struct scan_bitmap *bm, u64 head)
{
	u64 curr_p;
	u64 next;
...
static void bytefs_traverse_dir_inode_log(struct super_block *sb,
	struct bytefs_inode *pi, struct scan_bitmap *bm)
{
	bytefs_traverse_inode_log(sb, pi, bm, pi->log_head);
	if (metadata_csum)
		bytefs_traverse_inode_log(sb, pi, bm, pi->alter_log_head);
}

static unsigned int bytefs_check_old_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih, u64 entry_addr,
	...
	struct scan_bitmap *bm)
{
	...
	entry = (struct bytefs_file_write_entry *)entry_addr;
...
static int bytefs_set_ring_array(struct super_block *sb,
	struct bytefs_inode_info_header *sih, struct bytefs_file_write_entry *entry,
	struct bytefs_file_write_entry *entryc, struct task_ring *ring,
	unsigned long base, struct scan_bitmap *bm)
{
	...
	for (pgoff = start; pgoff < end; pgoff++) {
		...
		if (ring->nvmm_array[index]) {
			if (ring->entry_array[index] != old_entry) {
				if (old_entry)
					bytefs_check_old_entry(sb, sih, old_entry,
							old_pgoff, num_free,
							epoch_id, ring, base,
							bm);
	...
	if (old_entry)
		bytefs_check_old_entry(sb, sih, old_entry, old_pgoff,
...
static int bytefs_set_file_bm(struct super_block *sb,
	struct bytefs_inode_info_header *sih, struct task_ring *ring,
...
static void bytefs_ring_setattr_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih,
	struct bytefs_setattr_logentry *entry, struct task_ring *ring,
...
static unsigned long bytefs_traverse_file_write_entry(struct super_block *sb,
	struct bytefs_inode_info_header *sih, struct bytefs_file_write_entry *entry,
	struct bytefs_file_write_entry *entryc, struct task_ring *ring,
	unsigned long base, struct scan_bitmap *bm)
{
	...
	if (entryc->num_pages != entryc->invalid_pages) {
		...
		if (entryc->pgoff < base + MAX_PGOFF &&
			...
			bytefs_set_ring_array(sb, sih, entry, entryc,
						ring, base, bm);
	}
...
static int bytefs_traverse_file_inode_log(struct super_block *sb,
...
again:
	...
	while (curr_p != pi->log_tail) {
		...
		entry = (void *)bytefs_get_block(sb, curr_p);
		...
		switch (type) {
		case SET_ATTR:
			bytefs_ring_setattr_entry(sb, sih, SENTRY(entryc),
						ring, base, data_bits,
						bm);
		...
		case LINK_CHANGE:
			...
			break;
		case FILE_WRITE:
			curr_last = bytefs_traverse_file_write_entry(sb, sih, WENTRY(entry),
	...
	bytefs_set_file_bm(sb, sih, ring, bm, base, last_blocknr);
	if (last_blocknr >= base + MAX_PGOFF) {
		base += MAX_PGOFF;
		goto again;
	}
...
static int bytefs_recover_inode_pages(struct super_block *sb,
	struct bytefs_inode_info_header *sih, struct task_ring *ring,
	struct bytefs_inode *pi, struct scan_bitmap *bm)
{
	unsigned long bytefs_ino;

	if (pi->deleted == 1)
		return 0;

	bytefs_ino = pi->bytefs_ino;
	ring->inodes_used_count++;

	sih->i_mode = __le16_to_cpu(pi->i_mode);
	sih->ino = bytefs_ino;

	bytefs_dbgv("%s: inode %lu, head 0x%llx, tail 0x%llx\n",
			__func__, bytefs_ino, pi->log_head, pi->log_tail);

	switch (__le16_to_cpu(pi->i_mode) & S_IFMT) {
	case S_IFDIR:
		bytefs_traverse_dir_inode_log(sb, pi, bm);
		break;
	case S_IFLNK:
	...
	default:
		...
		bytefs_traverse_file_inode_log(sb, pi, sih, ring, bm);
		break;
	}
...
static inline int bytefs_failure_update_inodetree(struct super_block *sb,
	struct bytefs_inode *pi, unsigned long *ino_low, unsigned long *ino_high)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);

	if (*ino_low == 0) {
		*ino_low = *ino_high = pi->bytefs_ino;
	} else {
		if (pi->bytefs_ino == *ino_high + sbi->cpus) {
			*ino_high = pi->bytefs_ino;
		} else {
			/* A new start */
			bytefs_failure_insert_inodetree(sb, *ino_low, *ino_high);
			*ino_low = *ino_high = pi->bytefs_ino;
		}
	}
...
static int failure_thread_func(void *data)
{
	...
	pi = bytefs_get_inode_by_ino(sb, BYTEFS_INODETABLE_INO);
	data_bits = blk_type_to_shift[pi->i_blk_type];
	num_inodes_per_page = 1 << (data_bits - BYTEFS_INODE_BITS);
	...
	for (count = 0; count < ring->num; count++) {
		...
		for (i = 0; i < num_inodes_per_page; i++) {
			...
			if (fake_pi.i_mode && fake_pi.deleted == 0) {
				if (fake_pi.valid == 0) {
					// ret = bytefs_append_inode_to_snapshot(sb,
					// 				pi);
					if (ret != 0) {
						/* Deleteable */
						pi->deleted = 1;
				...
				bytefs_recover_inode_pages(sb, &sih, ring,
						&fake_pi, global_bm[cpuid]);
				bytefs_failure_update_inodetree(sb, pi,
						&ino_low, &ino_high);
	...
	if (max_size) {
		...
		bytefs_delete_file_tree(sb, &sih, 0, last_blocknr,
						false, false, 0);
	}
...
static int bytefs_failure_recovery_crawl(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct bytefs_inode_info_header sih;
	...
	root_addr = bytefs_get_reserved_inode_addr(sb, BYTEFS_ROOT_INO);
	...
	for (cpuid = 0; cpuid < sbi->cpus; cpuid++) {
		...
		for (version = 0; version < num_tables; version++) {
			inode_table = bytefs_get_inode_table(sb, version,
								cpuid);
			if (!inode_table)
				return -EINVAL;
			...
			while (curr) {
				if (ring->num >= 512) {
					bytefs_err(sb, "%s: ring size too small\n",
						 __func__);
					return -EINVAL;
				}
				...
				curr_addr = (unsigned long)bytefs_get_block(sb,
								curr);
	...
	ret = bytefs_get_reference(sb, root_addr, &fake_pi,
			(void **)&pi, sizeof(struct bytefs_inode));
	if (ret) {
		bytefs_dbg("Recover root pi failed\n");
		return ret;
	}

	bytefs_recover_inode_pages(sb, &sih, &task_rings[0],
					&fake_pi, global_bm[0]);

	return ret;
}

int bytefs_failure_recovery(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct task_ring *ring;
	...
	// i = BYTEFS_SNAPSHOT_INO % sbi->cpus;
	// pi = bytefs_get_inode_by_ino(sb, BYTEFS_SNAPSHOT_INO);
	// /* Set snapshot info log pages */
	// bytefs_traverse_dir_inode_log(sb, pi, global_bm[i]);
	...
	ret = bytefs_failure_recovery_crawl(sb);
	...
	bytefs_dbg("Failure recovery total recovered %lu\n",
...
static bool bytefs_try_normal_recovery(struct super_block *sb)
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct bytefs_inode *pi =  bytefs_get_inode_by_ino(sb, BYTEFS_BLOCKNODE_INO);
	int ret;
	...
	ret = bytefs_init_blockmap_from_inode(sb);
	if (ret) {
		bytefs_err(sb, "init blockmap failed, fall back to failure recovery\n");
		return false;
	}

	ret = bytefs_init_inode_list_from_inode(sb);
	if (ret) {
		bytefs_err(sb, "init inode list failed, fall back to failure recovery\n");
		bytefs_destroy_blocknode_trees(sb);
		return false;
	}

	if (sbi->mount_snapshot == 0) {
		// ret = bytefs_restore_snapshot_table(sb, 0);
		// if (ret) {
		// 	bytefs_err(sb, "Restore snapshot table failed, fall back to failure recovery\n");
		// 	bytefs_destroy_snapshot_infos(sb);
		// 	return false;
		// }
	}
...
{
	struct bytefs_sb_info *sbi = BYTEFS_SB(sb);
	struct bytefs_super_block *super = sbi->bytefs_sb;
	...
	BYTEFS_START_TIMING(recovery_t, start);
	sbi->num_blocks = ((unsigned long)(initsize) >> PAGE_SHIFT);

	/* initialize free list info */
	bytefs_init_blockmap(sb, 1);

	value = bytefs_try_normal_recovery(sb);
	if (value) {
		bytefs_dbg("BYTEFS: Normal shutdown\n");
	} else {
		bytefs_dbg("BYTEFS: Failure recovery\n");
		...
		if (sbi->mount_snapshot == 0) {
			...
			// ret = bytefs_restore_snapshot_table(sb, 1);
			// if (ret) {
			// 	bytefs_dbg("Initialize snapshot infos failed\n");
			// 	bytefs_destroy_snapshot_infos(sb);
			// 	goto out;
			// }
		}
		...
		sbi->s_inodes_used_count = 0;
		ret = bytefs_failure_recovery(sb);
		if (ret)
			goto out;

		ret = bytefs_build_blocknode_map(sb, initsize);
	}

out:
	BYTEFS_END_TIMING(recovery_t, start);
	if (measure_timing == 0) {
		getrawmonotonic(&end);
