...
static inline void
__nova_memunlock_range(void *p, unsigned long len, unsigned long act_len, unsigned long *flags)
{
	/*
	...
	NOVA_DECACHE_FLUSH_BYTE_ISSUE_LEN(p, cache_ptr, act_len, NOVA_ISSUE_ALL);
}
...
static inline void nova_memunlock_range(struct super_block *sb, void *p,
					 unsigned long len, unsigned long act_len, unsigned long *flags)
{
	// printk(KERN_ERR "hi");
	...
	if (nova_is_protected(sb))
		__nova_memunlock_range(p, len, act_len, flags);
}
...
static inline void nova_memunlock_super(struct super_block *sb, unsigned long *flags)
{
	...
	if (nova_is_protected(sb))
		__nova_memunlock_range(ps, NOVA_SB_SIZE, NOVA_SB_SIZE, flags);
}
...
static inline void nova_memunlock_journal(struct super_block *sb, unsigned long *flags)
{
	...
	if (nova_is_protected(sb))
		__nova_memunlock_range(addr, NOVA_DEF_BLOCK_SIZE_4K, sizeof(unsigned long *), flags);
}
					 ...
					 struct nova_inode *pi, unsigned long *flags)
{
	...
	if (nova_is_protected(sb))
		__nova_memunlock_range(pi, NOVA_INODE_SIZE, 120, flags);
}
...
static inline void nova_memunlock_block(struct super_block *sb, void *bp, unsigned long act_len, unsigned long *flags)
{
	if (nova_range_check(sb, bp, sb->s_blocksize))
		return;

	if (nova_is_protected(sb))
		__nova_memunlock_range(bp, sb->s_blocksize, act_len, flags);
}
