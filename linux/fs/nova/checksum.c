...
static int nova_repair_entry_pr(struct super_block *sb, void *entry)
{
	...
	nova_memunlock_range(sb, entry_pr, POISON_RADIUS, POISON_RADIUS, &irq_flags);
	...
	size_t entry_size)
{
	...
	nova_memunlock_range(sb, bad, entry_size, entry_size, &irq_flags);
	...
	struct nova_inode *bad_pi, struct nova_inode *good_pi)
{
	...
	nova_memunlock_range(sb, bad_pr, POISON_RADIUS, POISON_RADIUS, &irq_flags);
...
copy:
		...
		nova_memunlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN * 8, NOVA_DATA_CSUM_LEN * 8, &irq_flags);
		if (support_clwb) {
	...
	for (strp = 0; strp < strps; strp++) {
		...
		nova_memunlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN, NOVA_DATA_CSUM_LEN, &irq_flags);
	...
	for (strp = 0; strp < strps; strp++) {
		...
		if (csum_nvmm0 != csum_nvmm1) {
			...
			nova_memunlock_range(sb, csum_addr0,
							NOVA_DATA_CSUM_LEN, NOVA_DATA_CSUM_LEN, &irq_flags);
