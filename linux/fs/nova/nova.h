...
static inline void nova_memcpy_atomic(void *dst, const void *src, u8 size)
{
	...
	// However, if you do grep -r nova_memcpy_atomic in NOVA, it's not called.
	switch (size) {
	case 1: {
	...
	void *dram, void **nvmm, size_t size)
{
	...
	*nvmm = nova_get_block(sb, block);
	...
	NOVA_CACHE_BYTE_ISSUE_LEN((*nvmm), cache_ptr, ((u64)size), NOVA_INODE_ISSUE);
	NOVA_DECACHE_END_BYTE_ISSUE((*nvmm), cache_ptr);
...
static inline u64 next_log_page(struct super_block *sb, u64 curr)
{
	...
	u64* sim_ptr;
	NOVA_CACHE_BYTE_ISSUE(&curr_page->page_tail.next_page, sim_ptr, NOVA_LOG_ISSUE);
	NOVA_DECACHE_END_BYTE_ISSUE(&curr_page->page_tail.next_page, sim_ptr);
...
static inline u64 alter_log_page(struct super_block *sb, u64 curr)
{
	...
	u64* sim_ptr;
	NOVA_CACHE_BYTE_ISSUE(&curr_page->page_tail.alter_page, sim_ptr, NOVA_LOG_ISSUE);
	NOVA_DECACHE_END_BYTE_ISSUE(&curr_page->page_tail.alter_page, sim_ptr);
...
static inline bool goto_next_page(struct super_block *sb, u64 curr_p)
{
	...
	u8* sim_ptr;
	NOVA_CACHE_BYTE_ISSUE(addr, sim_ptr, NOVA_LOG_ISSUE);
