...
static int nova_readdir_fast(struct file *file, struct dir_context *ctx)
{
	...
	struct nova_inode *child_pi_c;
	struct nova_inode *prev_child_pi_c;
	...
	struct nova_inode *pidir_c;
	NOVA_CACHE_BYTE_ISSUE(pidir, pidir_c, NOVA_DIR_ISSUE);
	NOVA_DECACHE_END_BYTE_ISSUE(pidir, pidir_c);
	...
	while (curr_p != sih->log_tail) {
		...
		if (entryc->ino > 0 && entryc->invalid == 0
			...
			NOVA_CACHE_BYTE_ISSUE(child_pi, child_pi_c, NOVA_DIR_ISSUE);
			NOVA_DECACHE_END_BYTE_ISSUE(child_pi, child_pi_c);
			...
			NOVA_CACHE_BYTE_ISSUE(prev_child_pi, prev_child_pi_c, NOVA_DIR_ISSUE);
			NOVA_DECACHE_END_BYTE_ISSUE(prev_child_pi, prev_child_pi_c);
		}
		ctx->pos = pos;
		curr_p += de_len;
	}
