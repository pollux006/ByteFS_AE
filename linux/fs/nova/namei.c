				 ...
				 struct nova_dentry **res_entry)
{
	...
	*res_entry = direntry;
	...
	struct nova_dentry *direntryc_c;
	NOVA_CACHE_BYTE_ISSUE(direntryc, direntryc_c, NOVA_DIR_ISSUE);
	NOVA_DECACHE_END_BYTE_ISSUE(direntryc, direntryc_c);
			...
			bool excl)
{
	...
	pidir = nova_get_inode(sb, dir); // overhead added
	if (!pidir)
		goto out_err;
