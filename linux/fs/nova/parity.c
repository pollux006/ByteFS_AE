	...
	unsigned long blocknr, int zero)
{
	...
	nova_memunlock_range(sb, nvmmptr, strp_size, strp_size, &irq_flags);
	...
	size_t offset, size_t bytes)
{
	...
	if (unroll_csum || unroll_parity) {
		...
		if (data_csum > 0 && unroll_csum) {
			...
			nova_memunlock_range(sb, nvmmptr, csum_size * 8, csum_size * 8, &irq_flags);
	...
	u32 csum1, u32 *csum_good)
{
	...
	if (success) {
		...
		nova_memunlock_range(sb, stripptr, strp_size, strp_size, &irq_flags);
