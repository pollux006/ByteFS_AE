	...
	unsigned int num_free)
{
	...
	nova_memunlock_range(sb, entry, CACHELINE_SIZE, CACHELINE_SIZE, &irq_flags);
	...
	struct inode *inode, loff_t newsize)
{
	...
	nova_memunlock_range(sb, nvmm_addr + offset, length, length, &irq_flags);
	...
	struct nova_log_entry_info *entry_info)
{
	...
	nova_memunlock_range(sb, entry, size, size, &irq_flags);
	memset(entry, 0, size);
	...
	if (metadata_csum) {
		...
		nova_memunlock_range(sb, alter_entry, size, size, &irq_flags);
...
{
	unsigned long irq_flags = 0;
	nova_memunlock_range(sb, entry, sizeof(*entry), sizeof(*entry), &irq_flags);
	...
	unsigned long num_pages)
{
	...
	if (prev_blocknr) {
		...
		next_page = nova_get_block_off(sb, first_blocknr,
				NOVA_BLOCK_TYPE_4K);
		nova_memunlock_block(sb, curr_page, sizeof(struct nova_inode_page_tail), &irq_flags);
	...
	for (i = 0; i < num_pages - 1; i++) {
		next_page = nova_get_block_off(sb, next_blocknr,
				NOVA_BLOCK_TYPE_4K);
		nova_memunlock_block(sb, curr_page, 3 * sizeof(struct nova_inode_page_tail), &irq_flags);
	...
	int thorough_gc, int *extended)
{
	...
	if (curr_p == 0 || (is_last_entry(curr_p, size) &&
		...
		if (is_last_entry(curr_p, size)) {
			nova_memunlock_block(sb, nova_get_block(sb, curr_p), sizeof(u8), &irq_flags);
