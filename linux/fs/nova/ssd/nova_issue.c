...
#endif
	nova_ssd_init();
	if(issue_type==NOVA_DATA_BYTE_ISSUE){
		if(is_write){
			SSD_STAT_ATOMIC_ADD(byte_data_traffic_w,size);
		}else{
			SSD_STAT_ATOMIC_ADD(byte_data_traffic_r,size);
		}
	}else{
		if(is_write){
			SSD_STAT_ATOMIC_ADD(byte_metadata_issue_traffic_w,size);
		}else{
			SSD_STAT_ATOMIC_ADD(byte_metadata_issue_traffic_r,size);
		}
	}
