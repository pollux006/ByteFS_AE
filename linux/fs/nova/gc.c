	...
	struct nova_inode_log_page *last_page, u64 curr_head)
{
	...
	nova_memunlock_block(sb, last_page, sizeof(struct nova_inode_page_tail), &irq_flags);
	...
	unsigned long blocks, unsigned long checked_pages)
{
	...
	while (curr_p != sih->log_tail) {
		...
		if (!ret) {
			...
			nova_memunlock_block(sb, nova_get_block(sb, new_curr), length, &irq_flags);
	...
	nova_memunlock_block(sb, curr_page, sizeof(u8) + sizeof(struct nova_inode_page_tail), &irq_flags);
	...
	unsigned long blocks, unsigned long checked_pages)
{
	...
	new_curr = new_head;
	while (1) {
		nova_memunlock_block(sb, nova_get_block(sb, new_curr), sb->s_blocksize, &irq_flags);
		memcpy_to_pmem_nocache(nova_get_block(sb, new_curr),
	...
	nova_memunlock_block(sb, alter_curr_page, sizeof(struct nova_inode_page_tail), &irq_flags);
