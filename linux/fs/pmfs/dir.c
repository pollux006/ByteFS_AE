	...
	struct pmfs_direntry *de, u8 *blk_base,  struct pmfs_inode *pidir)
{
	...
	if (de->ino) {
		...
		pmfs_memunlock_block(dir->i_sb, blk_base,64);
	...
	pmfs_memunlock_block(dir->i_sb, blk_base,namelen+64);
	/*de->file_type = 0;*/
	if (inode) {
		...
		struct inode *inode)
{
	...
	pmfs_memunlock_block(sb, blk_base,64);
	de->ino = 0;
		...
		struct inode *inode)
{
	...
	#endif
		pmfs_memunlock_block(sb, blk_base,16);
		prev_entry->de_len =
