	...
	pmfs_logentry_t *le)
{
	...
	if (le->size > 0) {
		...
		pmfs_memunlock_range(sb, data, le->size,le->size);
		...
		pmfs_transaction_t *trans)
{
	...
	int i;
	char *data;
	pmfs_transaction_t *le_c; 
	for (i = 0; i < trans->num_used; i++, le++) {
		CACHE_BYTE_ISSUE(le, le_c);
		if (le->size) {
			data = pmfs_get_block(sb,le64_to_cpu(le->addr_offset));
			if (sbi->redo_log) {
				pmfs_memunlock_range(sb, data, le->size,le->size);
		...
		DECACHE_END_BYTE_ISSUE(le, le_c);
	}
}
		...
		pmfs_transaction_t *trans)
{
	pmfs_logentry_t *le = trans->start_addr;
	int i;
	// haor2 : overhead done by write
	pmfs_memunlock_range(sb, trans->start_addr,
			trans->num_entries * LOGENTRY_SIZE, trans->num_entries * LOGENTRY_SIZE);
	for (i = 0; i < trans->num_entries; i++) {
	...
	uint32_t tail, pmfs_logentry_t *le, bool recover, int *processed)
{
	...
	*processed = 0;
	...
	trans.start_addr = le;
	trans.gen_id = gen_id;
	// haor2 : the DECACHE_END_BYTE_ISSUE(le, le_c) will be called before exiting the loop
	// or when we proceed to a point where current version of le (before update) 
	// is no longer used
	do {
		trans.num_entries++;
		trans.num_used++;
	...
	int take_lock)
{
	...
	pmfs_logentry_t *le;
	pmfs_transaction_t *le_c; 
	pmfs_journal_t *journal_c;
	CACHE_BYTE_ISSUE(journal, journal_c);
	...
	while (head != tail) {
		...
		if (gen_id == le16_to_cpu(le->gen_id)) {
			...
			if (gen_id == MAX_GEN_ID) {
				pmfs_memunlock_range(sb, le, sizeof(*le), sizeof(*le));
				invalidate_gen_id(le);
	...
	PERSISTENT_MARK();
	PERSISTENT_BARRIER();
	pmfs_memunlock_range(sb, journal, sizeof(*journal), sizeof(*journal)); /* haor2 : this marks we will do a flush from DRAM to NVMM */
	...
	DECACHE_END_BYTE_ISSUE(journal, journal_c);
	return total;
}
	...
	uint32_t size)
{
	...
	pmfs_memunlock_range(sb, journal, sizeof(*journal), sizeof(*journal));
	...
	pmfs_memunlock_range(sb, sbi->journal_base_addr, size, size);
...
pmfs_transaction_t *pmfs_new_transaction(struct super_block *sb,
		int max_log_entries)
{
	// overhead is counted by memunlock : haor2
	pmfs_journal_t *journal = pmfs_get_journal(sb);
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	pmfs_transaction_t *trans;
...
{
	// haor2 : equivalent overhead done by memunlock
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	pmfs_logentry_t *le;
	int num_les = 0, i;
	...
	pmfs_memunlock_range(sb, le, sizeof(*le) * num_les, sizeof(*le) * num_les);
	for (i = 0; i < num_les; i++) {
	...
	void *journal_vaddr, uint32_t jtail, uint32_t jsize)
{
	...
	pmfs_memunlock_range(sb, start, jsize - jtail, jsize - jtail);
	while (jtail < jsize) {
		invalidate_gen_id(le);
		le++;
		jtail += LOGENTRY_SIZE;
...
{
	// haor2 : overhead done by memublock (journal)
