...
static inline void
__pmfs_memunlock_range(void *p, unsigned long len, unsigned long act_len)
{
	/*
	 ...
	 */
	void* cache_ptr = pmfs_dummy_buffer;
	...
	DECACHE_FLUSH_BYTE_ISSUE_LEN(p, cache_ptr, act_len);
}
...
static inline void pmfs_memunlock_range(struct super_block *sb, void *p,
					 unsigned long len, unsigned long act_len)
{
	if (pmfs_is_protected(sb))
		__pmfs_memunlock_range(p, len, act_len);
}
...
{
	if (pmfs_is_protected(sb))
		__pmfs_memunlock_range(ps, PMFS_SB_SIZE, PMFS_SB_SIZE);
}
...
{
	if (pmfs_is_protected(sb))
		__pmfs_memunlock_range(pi, PMFS_SB_SIZE, 96);
}
...
static inline void pmfs_memunlock_block(struct super_block *sb, void *bp, unsigned long act_len)
{
	if (pmfs_is_protected(sb))
		__pmfs_memunlock_range(bp, sb->s_blocksize, act_len);
}
