	...
	if (all_range_freed &&
		...
		*meta_empty = true;
		...
		if (start <= end) {
			...
			pmfs_memunlock_block(sb, node,bzero);
		...
		struct pmfs_inode *pi, u32 new_height)
{
	...
	while (height < new_height) {
		...
		pmfs_memunlock_block(sb, root,64);
		root[0] = prev_root;
	...
	for (i = first_index; i <= last_index; i++) {
		if (height == 1) {
			if (node[i] == 0) {
				...
				pmfs_memunlock_block(sb, node,64);
...
static void pmfs_block_truncate_page(struct inode *inode, loff_t newsize)
{
	...
	if (!bp)
		return;
	pmfs_memunlock_block(sb, bp,length);
...
void pmfs_truncate_del(struct inode *inode)
{
	...
	if (prev == &sbi->s_truncate) {
		...
		pmfs_memunlock_range(sb, li_prv, sizeof(*li_prv), 16);
...
void pmfs_truncate_add(struct inode *inode, u64 truncate_size)
{
	...
	pmfs_memunlock_range(sb, li, sizeof(*li),16);
