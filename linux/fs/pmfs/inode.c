		...
		unsigned long *blocknr, int zero)
{
	...
	if (!errval) {
		...
		//@TODO atomic increase on count
		pmfs_ssd_write(pi,64); 
	}
	...
	if (all_range_freed &&
		...
		*meta_empty = true;
		...
		if (start <= end) {
			...
			// @TODO zero out should use trim to do it
			//@ TODO stat the bzero here
			// pmfs_ssd_write(pi,64); 
		}
		*meta_empty = false;
	}
		...
		struct pmfs_inode *pi, u32 new_height)
{
	...
	while (height < new_height) {
		...
		//@ althought it is 8B we need to flush a lot
		pmfs_ssd_write(root,64); 
	...
	bool zero)
{
	...
	|| trans == NULL ) {
		...
		// @TODO this shoul not be correct, node size is 8B 
		pmfs_ssd_write(&node[first_index],64*(flush_bytes/64)); 
...
static int pmfs_read_inode(struct inode *inode, struct pmfs_inode *pi)
{
	...
	//@ TODO insert inode cache here, if not evicted, this should be the only read 
...
static int pmfs_increase_inode_table_size(struct super_block *sb)
{
	...
	if (errval == 0) {
		...
		pmfs_memlock_inode(sb, pi);
		//@TODO change inode size, can this be delayed?
		pmfs_ssd_write(pi,64); 
...
static void pmfs_block_truncate_page(struct inode *inode, loff_t newsize)
{
	...
	pmfs_ssd_write(bp + offset,length); 
