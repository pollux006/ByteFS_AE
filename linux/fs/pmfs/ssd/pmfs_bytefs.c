...
int32_t pmfs_data_issue(int is_write, uint64_t pfn, uint64_t offset, 
 uint64_t size, void* buf, struct file *filp) {
	...
	// here based on the pfn, we check if it is in cache
	...
	if (!is_write)
		return 0;
	// if (is_write)
	// 	return nvme_issue(1, pfn, (size/4096), buf);
	return _byte_issue(is_write, lpa, size, buf);
#else
	...
	// change to simple issue here
	uint64_t cur, left, nextwrite;
	uint64_t curpfn;
	curpfn = lpa/PG_SIZE;
	cur = lpa%PG_SIZE;
	left = size;
	...
	while (left > 0) {
		if (cur > 0) {
			nextwrite = (cur + left > PG_SIZE) ? PG_SIZE - cur : left;
			_byte_issue(is_write, curpfn * PG_SIZE + cur, nextwrite, buf);
			left -= nextwrite;
			cur = 0;
			curpfn++;
		} else if (cur == 0) {
			if (left >= PG_SIZE) {
				// start with block aligned
				nextwrite = (left / PG_SIZE) * PG_SIZE;
				// nvme_issue(is_write, curpfn, nextwrite / PG_SIZE, buf + size - left);
				left -= nextwrite;
				curpfn += nextwrite / PG_SIZE;
			} else {
				_byte_issue(is_write, curpfn * PG_SIZE, left, buf + size - left);
				left =0;
			}
		} 
		// for full block inside
