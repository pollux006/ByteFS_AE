...
#define PA_START (BYTEFS_PA_START)
#define PA_END   (BYTEFS_PA_END) 
...
do {	                                                                              \
	if (cache_ptr == NULL)                                                            \
		printk(KERN_ERR "%s : null ptr", __func__);                                     \
	else if (0 > pmfs_byte_issue(1, page_to_pfn(virt_to_page(virt_addr)),             \
					 pmfs_virt_to_offset(virt_addr), len, cache_ptr)) {                       \
		printk(KERN_ERR "%s : write failed page %lu offset %lu len %lu", __func__,      \
				   page_to_pfn(virt_to_page(virt_addr)),                                    \
