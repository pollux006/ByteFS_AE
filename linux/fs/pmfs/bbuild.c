	...
	struct super_block *sb, struct pmfs_inode *pi, unsigned long num_blocks)
{
	...
	pmfs_memlock_inode(sb, pi);
	pmfs_ssd_write(pi,64); //@ TODO write half inode
...
void pmfs_save_blocknode_mappings(struct super_block *sb)
{
	...
	list_for_each_entry(i, head, link) {
		...
		//@NOTICE 16B data structure in block of 4k
		...
		if (j == 256) {
			...
			pmfs_ssd_write(p,4096);  // @TODO actually should use block interface?
	...
	//@TODO update super block  this is wierd since sb is 82 BYTE?
	pmfs_ssd_write(super,128); 
#ifdef ENABLE_JOURNAL
	/* commit the transaction */
	pmfs_commit_transaction(sb, trans);
#endif
