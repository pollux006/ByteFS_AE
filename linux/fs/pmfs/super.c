				      ...
				      unsigned long size)
{
	...
	pmfs_memunlock_range(sb, super, journal_data_start,journal_data_start);
#endif
	...
	pmfs_memunlock_range(sb, super, PMFS_SB_SIZE*2,PMFS_SB_SIZE*2);
	...
	pmfs_memunlock_range(sb, de, sb->s_blocksize,256);
...
static void pmfs_recover_truncate_list(struct super_block *sb)
{
	...
	pmfs_memunlock_range(sb, head, sizeof(*head),16);
...
setup_sb:
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		pmfs_memunlock_range(sb, &super->s_mtime, 8,8);
...
int pmfs_remount(struct super_block *sb, int *mntflags, char *data)
{
	...
	if ((*mntflags & MS_RDONLY) != (sb->s_flags & MS_RDONLY)) {
		...
		pmfs_memunlock_range(sb, &ps->s_mtime, 8,8);
