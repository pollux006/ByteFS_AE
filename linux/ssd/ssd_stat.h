...
struct ssd_stat {
    ...
    atomic_long_t page_cache_flush_traffic;
    atomic_long_t page_cache_actuall_w_traffic;
    // locality related
    atomic_long_t locality_info[PG_SIZE / BYTEFS_LOG_REGION_GRANDULARITY][LOC_MAX][2]; 
    // temp/test
    atomic_long_t temp[25];
};


extern struct ssd_stat stat;

extern inline int check_stat_state(void);
