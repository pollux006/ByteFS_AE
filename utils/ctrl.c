...
int main(int argc, char *argv[]) {
  ...
  switch (opt) {
    ...
    case 42:{
      ...
      // log based stats
      const int short_field_len = 10;
      const int long_field_len = 20;
      printf("  Issue count\n");
      ...
      printf("  Paged\n");
      printf("    Buffer read hit:        %-*lu\n", long_field_len, stat.buffer_rd_hit);
      printf("    Buffer write hit:       %-*lu\n", long_field_len, stat.buffer_wr_hit);
      printf("    Buffer read miss:       %-*lu\n", long_field_len, stat.buffer_rd_miss);
      printf("    Buffer write miss:      %-*lu\n", long_field_len, stat.buffer_wr_miss);
      printf("    Buffer promote pages:   %-*lu\n", long_field_len, stat.buffer_promote);
      printf("    Buffer evict pages:     %-*lu\n", long_field_len, stat.buffer_evict);

      printf("  Page Cache\n");
      printf("   Cache Read:              %-*lu = Hit: %-*lu + Miss: %-*lu \n", 
      ...
      printf("   Cache Write:             %-*lu = Hit: %-*lu + Miss: %-*lu \n", 
